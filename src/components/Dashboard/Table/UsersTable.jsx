...
const UsersTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessorFn: (row) => row.placedOrders.length > 0 ? row.placedOrders.map(placedOrder => placedOrder._id).join(', ') : 'No Orders',
      footer: 'Placed Orders'
    },
  ]
  ...
  return (
    <div className='container p-4 h-full'>
      <div className='bg-white p-8 rounded-md h-full overflow-scroll'>
        ...
        { loading ? <div className="flex justify-center items-center text-center"><Triangle height={569} width={300} color="black" ariaLabel="triangle-loading" wrapperStyle={{}} visible={true} /></div> : users && <table className='h-full w-full'>
          <thead>
          ...
          <tbody>
            { users && users.length > 0 && table.getRowModel().rows.map(row => (
              <tr key={row.id}>
