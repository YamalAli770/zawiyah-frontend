...
const CartsTable = () => {
  ...
  const data = useMemo(() => carts, [carts]);
  ...
  const columns = [
    ...
    {
      header: 'Cart Owner (User ID)',
      accessorKey: 'cartOwner',
      footer: 'Cart Owner (User ID)'
    },
    {
      header: 'Cart Items',
      accessorFn: (row) => row.cartItems.length > 0 ? row.cartItems.map(cartItem => cartItem.name).join(', ') : 'No Products',
  ...
  return (
    <div className='container p-4 h-full'>
      <div className='bg-white p-8 rounded-md h-full overflow-scroll'>
        ...
        { loading ? <div className="flex justify-center items-center text-center"><Triangle height={569} width={300} color="black" ariaLabel="triangle-loading" wrapperStyle={{}} visible={true} /></div> : carts && <table className='h-full w-full'>
          <thead>
          ...
          <tbody>
            { carts && carts.length > 0 && table.getRowModel().rows.map(row => (
              <tr key={row.id}>
