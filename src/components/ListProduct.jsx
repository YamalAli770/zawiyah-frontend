...
const ListProduct = () => {
  ...
  const handleImageChange = async (event) => {
    ...
    setImage(file);
    setDemoImage(URL.createObjectURL(file));
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = await uploadFile(image, { publicKey: 'e7622d60287398e37912', store: 'auto' })
  ...
  return (
      <>
        { loading ? <div className="flex justify-center items-center"><Triangle {...TRIANGLE_LOADER_CONFIG} /></div> : <section className="text-customText body-font overflow-hidden">
